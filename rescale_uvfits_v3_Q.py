# For use with Q-band observations after 2015-09 (a.k.a. since KVN-2015B)
...
###############################################
# Edit here
file_name_mas = 'IMOGABA37_Q_0235+164'
infile_uvfits = '{0}.KVN26'.format(file_name_mas,)
otfile_uvfits = '{0}_AeffCorr2.KVN26'.format(file_name_mas,)
###############################################
...
def kys_anomoly(obs_date):
    ...
    if len(kvn_epoch_final) == 0:
        ...
        return (kvn_epoch_final['Epoch'][0], 1)#kvn_epoch_final['KYS_SF'][0])
...
def scale_data(file_name, save_name):
    ...
    print('Found source {0}, observed on {1} ({2})'.format(obs_source, obs_date, obsepochif[0]))
    ...
    # Will eventually update to query table
